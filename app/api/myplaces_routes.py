...
@login_required
def addMyPlaces(id):
    existingPlaces = MyPlace.query.filter_by(campsiteid = id, userid = current_user.id).first()
    if existingPlaces is not None:
        return {'error': 'Campsite has already been added to My Places'}
